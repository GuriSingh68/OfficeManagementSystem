...
const FetchTasks = () => {
  ...
  { tasks.length !=0  ? <div className="space-y-4">
    {
      role==="admin" ? <div>{<Link href="/users/task" className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">Create task</Link>}</div> :
    tasks.slice(0, 2).map((task, index) => (
      <div key={index} className="bg-white p-6 rounded-lg shadow-lg max-w-sm mx-auto">
        ...
        {role==="admin" ? <div className="card-footer mt-4">
  ...
  </div> : <h3 className="text-xl font-semibold mb-4 hover:text-red-500">List empty</h3> }
    </div>
  );
};
